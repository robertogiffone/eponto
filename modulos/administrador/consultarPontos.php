...
if( $usuario->getPrivilegio() == 'Comum' ){ //Para tratar que usuário comum não tenha acesso a página de administrador
    header('Location:../logado/consultaPontos.php?consultar=0');
}
...
if ( $_GET['consultar']==1 ){
	...
	if ( empty($_POST['periodo']) ){
        $_SESSION['msg'] = 'Preencha a data inicial e a data final';
    }else{
        //$ordenaDados = 10; //Vai conter os valores da coluna responsável por ordenar os dados em ordem decrescente
        //$dataInicio = $objGeral->formataDataBanco($_POST['dataInicial']); 
        //$dataFinal = $objGeral->formataDataBanco($_POST['dataFinal']);
        ...
        $selecionar->setCampos('id');
        $usuario_id = $selecionar->buscaIdUsuario($_POST['usuario']);
        ...
        $pontos = $selecionar->consultaPontosPorPeriodoEUsuario($usuario->getEmpresa(),$usuario_id,$_POST['usuario'], $dataInicio, $dataFinal, $usuario->getPrivilegio() );
					...
					<section class="content">
						...
						<div class="row">
							...
							<div class="col-md-6 col-md-offset-3">
								<div class="box box-primary">
									...
									<form name="consultarPontos" id="consultarPontos" onsubmit="return validaConsultarPontos();" action="consultarPontos.php?consultar=1" method="post">
									
										<div class="box-body">
											<div class="form-group">
												<label> Usuário: </label> 
												<div class="input-group">
													<input type="text" name="usuario" id="usuario" class="campos" onkeypress="return somenteLetras(event.keyCode, event.which);" />
												</div><!-- /.input group -->
											</div><!-- /.form group -->
						...
						<?php
							if( isset($pontos) && $_GET['consultar'] == 1 ){
								include_once 'listaConsultarPontos.php';
